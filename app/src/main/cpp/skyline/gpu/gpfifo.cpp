        ...
        if (!gpEntry.size) {
            ...
            switch (gpEntry.opcode) {
                case GpEntry::Opcode::Nop:
                    return;
                default:
                    state.logger->Warn("Unsupported GpEntry control opcode used: %u", static_cast<uint>(gpEntry.opcode));
                    return;
            }
        }
        ...
        for (auto entry{pushBufferData.begin()}; entry != pushBufferData.end(); entry++) {
            ...
            switch (methodHeader.secOp) {
                ...
                case PushBufferMethodHeader::SecOp::EndPbSegment:
                    return;
                default:
                    state.logger->Warn("Unsupported pushbuffer method SecOp: %u", static_cast<uint>(methodHeader.secOp));
                    break;
            }
        }
    }
